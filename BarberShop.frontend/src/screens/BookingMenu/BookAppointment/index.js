...
export default function BookAppointment({ navigation }) {
	...
	const handleBooking = async () => {
		...
		try {
			let response = await FetchAPI("POST", "appointment", "book", { Year: year, Month: month, Day: day, Hour: hours, Minutes: minutes, PhoneNumber: customer?.PhoneNumber, FullName: `${customer?.FirstName} ${customer?.LastName}` });
			if (response.status === OK) {
				setActivity(false);
