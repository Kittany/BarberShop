...
export default function CustomersAppointments({navigation}) {
	const {
		authState: {customer},
	} = GlobalData?.();
		...
		try {
			...
			if (response.status === OK) {
				...
				await appointmentsInDatabase.forEach(item => {
					...
					if (appointmentDate.getDate() >= today.getDate() && appointmentDate.getMonth() >= today.getMonth()) 
					bookedAppointments.push({phoneNumber: item.PhoneNumber, fullName: item.FullName, day: dayNames[(appointmentDate.getDate() - 1) % 7], date: tempAppointmentArray[0], time: tempAppointmentArray[1]});
				});
